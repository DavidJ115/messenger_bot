...
router.post("/webhook", async (req, res) => {
  ...
  if (body.object === "page") {
    body.entry.forEach(async (entry) => {
      ...
      // // Intercepciones para consultas manuales del flujo
        if (lowerText.includes("informacion") || lowerText.includes("información") || lowerText.includes("asesor")) {
          botReply = { 
            text: "Agradecemos tu interés 😊\nSelecciona tu departamento o presiona en 'Otro...' para escribirlo:",
            quick_replies: departamentos
          };
          userStates[senderId].stage = "esperando_departamento";
        } 
        else if (lowerText.includes("requisitos")) {
          botReply = {
            text: "Requisitos para ingresar al CAE 📋:\n1️⃣ Edad: 18 a 38 años \n2️⃣ Haber completado la secundaria o tercer año del ciclo común \n3️⃣ Vocación de servicio \n\n"+
                  "Documentos para matrícula 📋:\n✔️ 2 copias del título secundaria o certificado del tercer año de ciclo común, ambas caras \n✔️ 2 copias ampliadas del DNI \n✔️ Hoja de antecedentes penales vigentes \n✔️ Hoja de antecedentes policiales \n2 fotografías tamaño carnet \n\n"+
                  "¿Te puedo ayudar con algo más? 😊",
            quick_replies: mainOptions
          };
          userStates[senderId].stage = "main_menu";
        } 
        else if (lowerText.includes("ubicacion") || lowerText.includes("ubicaciones") || lowerText.includes("ubicados") || lowerText.includes("ubicación")) {
          botReply = { 
            text: "CAE cuenta con 9 sedes a nivel nacional\n\n📍 Selecciona la ubicación más conveniente:", 
            quick_replies: sedes
          };
        ...
        const cancelKeywords = ["cancelar", "adios", "adiós", "hasta luego", "salir", "libre"];
        if (cancelKeywords.some(k => text.includes(k))) {
          userStates[senderId] = { stage: null, departamento: null, telefono: null };
          botReply = {text: `¡Entendido ${userName}! Espero haberte sido útil 👋 \nSi necesitas aún información solo di hola`,  quick_replies: mainOptions}; 
        } 
        else {
          ...
          const responses = [
            { keywords: ['hola', 'buenos días', 'buenos dias', 'buenas noches', 'buen dia', 'buenas tardes', 'buenas'], reply: `¡Hola ${userName}! 👋 Bienvenido al Chat Virtual de CAE\n\nSelecciona una opción o di Salir para escritura libre:` }
          ];
            ...
            for (const r of responses) {
              for (const keyword of r.keywords) {
                if (text.includes(keyword)) {
                  botReply = {text: r.reply,  quick_replies: mainOptions};
                  break;
                }
              }
              if (botReply) break;
          ...
          if (!botReply) {
            if (botReply && typeof botReply === "string" && botReply.includes("Selecciona una opción")) {
              ...
              if (text === "mayor información" || text === "mayor_informacion" || text === "informacion" || text === "mayor informacion" || lowerText.includes("informacion") || lowerText.includes("información")) {
                botReply = { 
                  text: "Agradecemos tu interes 😊 \n Selecciona tu departamento o presiona en 'Otro...' para escribirlo:",quick_replies: departamentos};
                  userStates[senderId].stage = "esperando_departamento";

              //Llamada al mensaje predeterminado si el usuario quiere saber los requisitos    
              } else if (text === "requisitos" || lowerText.includes("requisitos")) {
                botReply = {
                  text: "Requisitos para ingresar al CAE 📋:\n1️⃣ Edad: 18 a 38 años \n2️⃣ Haber completado la secundaria o tercer año del ciclo común \n3️⃣ Vocación de servicio \n\n"+
              ...
              }else if (text === "ubicaciones" || lowerText.includes("ubicacion") || lowerText.includes("ubicaciones") || lowerText.includes("ubicados") || lowerText.includes("ubicación")){
                botReply = {text: "CAE cuenta con 9 sedes a nivel nacional\n\n📍 Selecciona la ubicación más conveniente:", quick_replies: sedes};
                userStates[senderId].stage = "esperando_ubicacion";
              }
            }
            //Llamada para que el usuario obtenga información de cada sede si ya seleccionó alguna
              ...
              if (key && ubicaciones[key]) {
                ...
                botReply = {
                  text: "Estas son nuestras sedes 👇  \n\nSelecciona la más conveniente para saber más información", quick_replies: sedes};
              }
            ...
            if (!botReply || (typeof botReply === "string" && botReply.trim() === "")) {
              ...
              //Respuesta IA e impresión de menú inicial
              botReply = { text: aiReply, quick_replies: mainOptions }; 
            }
          }
      }
